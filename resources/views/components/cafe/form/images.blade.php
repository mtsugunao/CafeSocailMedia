<div x-data="inputFormHandler()" class="my-2">
    <template x-for="(field, i) in fields" :key="i">
        <div class="w-full flex my-2">
            <label :for="field.id" class="border border-gray-300 rounded-md p-2 w-full bg-white cursor-pointer">
                <input type="file" accept="image/png, image/jpeg" class="sr-only overflow-hidden" :id="field.id" name="image" @change="fields[i].file = $event.target.files[0]">
    ...
    <template x-if="fields.length < 1">
        <button type="button" @click="addField()" class="inline-flex font-medium rounded-md text-white bg-gray-500 hover:bg-gray-600 justify-center py-2 px-4 border border-transparent shadow-sm text-sm">
