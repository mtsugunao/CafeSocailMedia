<div class="flex">
    <div x-data="{ dropdownOpen: false }" class="relative items-center">
        <button @click="dropdownOpen = !dropdownOpen" class="relative z-10 inline-flex justify-center items-center px-5 h-full" x-on:click.away="dropdownOpen = false">
        ...
        <div x-show="dropdownOpen" class="fixed right-1/2 transform translate-x-1/2 sm:translate-x-0 w-4/5 sm:absolute sm:right-0 mt-2 bg-white rounded-md shadow-lg overflow-hidden z-20 sm:w-80">
            <div class="py-2">
                ...
                <a href="{{ $notification->data['url'] }}" class="justify-between flex items-center border-b px-4 py-3 hover:bg-gray-100">
                        <div class="flex items-center">
                            <img class="hidden sm:block h-8 w-8 rounded-full object-cover mx-1" src="{{ isset($notification->data['user_profile']) ? image_url_profiles($notification->data['user_profile']) : asset('images/user_icon.png') }}" alt="">
